...
def _validate_args(args):
    ...
    if args.dit_resident_dtype == "bf16" and args.lora_path is not None:
        raise ValueError(
            "--dit_resident_dtype bf16 requires an offline fused BF16 "
            "checkpoint and cannot be combined with --lora_path."
        )
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        default="fp32",
        help=(
            "Resident dtype of the SCAIL DiT checkpoint and parameters. "
            "BF16 requires an offline fused BF16 checkpoint."
        ))
    parser.add_argument(
        "--sample_solver",
        type=str,
...
def generate_video(pipeline: wan.SCAIL2Pipeline, prompt: str, image_path: str, image_mask_path: str, pose_path: str, driving_mask_path: str, args, device, rank, cfg, input_idx, replace_flag, additional_task_input=None, output_fps=None, output_fps_fraction=None):
    ...
    # Decode directly into one target-resolution uint8 CPU tensor in small
    # source-resolution batches. The pipeline normalizes only the active
    # segment to FP32, which is numerically identical to the legacy formula but
    # avoids materializing a full 4K sequence and a full-sequence FP32 copy on
    # every FSDP rank.
    pose_video = load_video_for_pose_sample(
        pose_path, image_size=(target_h, target_w)
    )  # t c h w, uint8
    ...
    if rank == 0:
        ...
        if written_video is None:
            raise RuntimeError(f"Failed to encode generated video: {save_file}")
...
def generate(args):
    ...
    scail_pipeline = wan.SCAIL2Pipeline(
        ...
        dit_resident_dtype=args.dit_resident_dtype,
    )
